...
const products = [{title: "asdadasd", price: 21312, id: 0}, {title: "asdsada", price: 2139, id: 1}];
...
class Contenedor {
  ...
  async save(object) {
    try {
      ...
      await fs.promises.appendFile(`./${this.fileName}.txt`,JSON.stringify(products, null, 2));
  ...
  async getById(number) {
    ...
    async deleteById(number){
        try{
         ...
         const newJson =  await fs.promises.writeFile(`./${this.fileName}.txt`,JSON.stringify(filter, null, 2))
        }catch(error){
             throw new Error (error)
         }
    }
